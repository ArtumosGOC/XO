...
void Jogo::iniciar() {
    ...
    do {
        ...
        for (int i = 0; i < 9; i++) {
            if (jogadorAtual == 1) {
                std::cout << "e a vez do jogador " << jogadorAtual << ". Escolha uma posicao: ";
                int posicao;
                std::cin >> posicao;

                if (posicao < 1 || posicao > 9) {
                    std::cout << "Posicao invalida! Tente outra posicao!\n";
                    i--;
                    continue;
                }

                if (!tabuleiro.colocarMarcador(posicao, jogador1.getMarcador())) {
                    std::cout << "Posicao ocupada! Tente outra posicao!\n";
                    i--;
                    continue;
                }
            }
            ...
            if (jogador_vencedor == 1) {
                std::cout << "Jogador " << jogadorAtual << " venceu! Parabens!\n";
                break;
            }
